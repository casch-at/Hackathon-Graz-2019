...
if test -e ~/.mk_infineon_ros.conf; then
    source ~/.mk_infineon_ros.conf
fi
...
HW_BVENDOR="$(cat /sys/devices/virtual/dmi/id/sys_vendor)"
...
case "${HW_BVENDOR}:${HW_PNAME}" in
GIGABYTE:GB-BXBT-1900)
    # CPUs 0,1 and 2,3 share caches; separating them will cause large latency spikes
    RT_CPUS=${RT_CPUS:-2,3}
...
# - Suite:  stretch, xenial, etc.
DEBIAN_SUITE=${DEBIAN_SUITE:-bionic}
...
TAG=${TAG:-$ROS_DISTRO-drone-$DEBIAN_SUITE}
